  ...
  try {
    ...
    if (admin.isAdmin) {
      ...
      for (let i = 0; i < buzzes.length; i++) {
        ...
        if (buzz.rebuzz != 0 && buzz.rebuzz != -1) {
          ...
          if (rebuzzPost.userid == targetid) {
            await Buzzes.updateOne({ _id: buzz._id }, { $set: { rebuzz: -1 } });
          }
        }
      ...
      try {
        ...
        console.log("Fail to Sent Email:", err);
      }
...
router.post("/deletebuzz", upload.none(), async (req, res) => {
  ...
  try {
    ...
    if (admin.isAdmin) {
      ...
      res.send({
        state: false,
        message: "You do not have permission to delete buzz.",
      });
    }
  } catch (err) {
    console.error(err);
  ...
  try {
    ...
    if (admin.isAdmin) {
      ...
      if (target) {
        ...
        res.send({
          state: true,
          message: `User with userid ${targetid} has been updated.`,
        });
      } else {
        res.send({
          state: false,
          message: `User with userid ${targetid} does not exist.`,
        });
      }
    } else {
      res.send({
